...
void parser_echo_result( int16_t rescode ) {
	...
	switch (rescode) {
	    case PARSE_WSPACE:
		if (parser_verbosity>1) printf_P(PSTR(" . . . . . whitespaces . . . . \n"));
		break;
	    case PARSE_COMMENT:
		if (parser_verbosity>1) printf_P(PSTR("# .-.-.-. comment line .-.-.-.\n"));
		break;
	    case PARSE_META:
...
void print_listing() {
	...
	strncpy_P( cache, opcode_list[op].name,6 );
	printf_P(PSTR("%5s "), cache);
	...
	if (parser_verbosity>1) { 
	    for (uint8_t n=0;n<(5-nest_level);n++) {
		printf_P(PSTR("   "));
	    }
	...
	nest_level += pgm_read_byte(&opcode_list[op].nest);
	curpos += size;
	curcount++;
    }
